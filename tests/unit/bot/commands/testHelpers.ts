...
export function createMockSleepCoreAPI(overrides: Partial<SleepCoreAPI> = {}): SleepCoreAPI {
  return {
    ...
    recordGamificationAction: jest.fn().mockResolvedValue({
      xpGained: 10,
      newBadges: [],
      questProgress: [],
      streakUpdates: [],
    ...
    recordDailyCheckIn: jest.fn().mockResolvedValue({
      ...
      newBadges: [],
      questProgress: [],
      streakUpdates: [{ type: 'daily_login', currentCount: 3, longestCount: 5 }],
      leveledUp: false,
      level: 2,
      awardedBadges: [],
    }),
    getUserBadges: jest.fn().mockResolvedValue([
      { badgeId: 'badge1', oderId: '123', earnedAt: new Date(), displayOrder: 0, isNew: false },
    ]),
    ...
    startQuest: jest.fn().mockResolvedValue({ id: 'quest1', questId: 'first_week', userId: 123, startedAt: new Date(), progress: {} }),
    getAvailableQuests: jest.fn().mockResolvedValue([
      { id: 'first_week', title: 'Первая неделя', description: 'Веди дневник 7 дней', icon: '📅', category: 'onboarding', difficulty: 'easy', reward: { xp: 100, badge: 'week1' } },
    ]),
    getCompletedQuestCount: jest.fn().mockResolvedValue(2),
    ...
    getStreaks: jest.fn().mockResolvedValue([
      { type: 'daily_login', currentCount: 3, longestCount: 5, isFrozen: false, multiplier: 1 },
      { type: 'sleep_diary', currentCount: 5, longestCount: 7, isFrozen: false, multiplier: 1.2 },
    ]),
    getGamificationSettings: jest.fn().mockResolvedValue({
