...
exports.TemporalEchoEngine = void 0;
exports.createTemporalEchoEngine = createTemporalEchoEngine;
const ITemporalPrediction_1 = require("./ITemporalPrediction");
/**
 * Temporal Echo Engine Implementation
 */
class TemporalEchoEngine {
    config;
    constructor(config = {}) {
        this.config = { ...ITemporalPrediction_1.DEFAULT_TEMPORAL_CONFIG, ...config };
    }
    /**
     * Generate state trajectory predictions
     */
    async predictTrajectory(currentState, stateHistory, horizons = ['6h', '12h', '24h', '72h']) {
        ...
        const vulnerabilityWindows = this.detectVulnerabilityWindows({ predictions });
        // Detect patterns
     ...
     */
    async predictAtHorizon(currentState, horizon) {
        const hoursAhead = ITemporalPrediction_1.HORIZON_HOURS[horizon];
     ...
     * Detect vulnerability windows
     */
    detectVulnerabilityWindows(trajectory, options = {}) {
        const windows = [];
        ...
        return windows.filter(w => !options.windowTypes || options.windowTypes.includes(w.type));
    }
    /**
     * Analyze circadian rhythm
     */
    async analyzeCircadianRhythm(stateHistory, minDays = 7) {
     ...
     */
    detectEarlyWarnings(stateHistory, windowSize = 10) {
        const signals = [];
        ...
        const signChanges = this.countSignChanges(values.map((v, i) => i > 0 ? v - values[i - 1] : 0));
     ...
     */
    async estimateTimeToState(currentState, targetCondition) {
     ...
     */
    async getOptimalInterventionTiming(currentState, interventionType) {
        const now = new Date();
     ...
     */
    extractTimeSeries(states, extractor) {
        return states.map(s => ({
     ...
     */
    kalmanUpdate(state, measurement, Q = this.config.kalmanParams.processNoise, R = this.config.kalmanParams.measurementNoise) {
        // Predict
     ...
     * Calculate EWMA
     */
    ewma(values, alpha = this.config.ewmaAlpha) {
        if (values.length === 0)
            return 0;
        let result = values[0];
     ...
     */
    predictWithDecay(current, mean, decayFactor) {
        return mean + (current - mean) * decayFactor;
    }
    /**
     * Calculate overall trend
     */
    calculateOverallTrend(series) {
        if (series.length < 3)
            return 'unknown';
     ...
     */
    identifyContributingFactors(state) {
        const factors = [];
        // Emotional factors
     ...
     */
    getRecommendedInterventions(prediction) {
        const interventions = [];
     ...
     * Detect phase transitions
     */
    detectPhaseTransitions(history, current) {
        const transitions = [];
     ...
     */
    buildRiskTrajectory(current, predictions) {
        return {
     ...
     */
    calculateOptimalWindows(hourlyProfile) {
        const windows = [];
        // Find high wellbeing, low variability times
        const goodHours = hourlyProfile.filter(h => h.avgWellbeing > 0.5 && h.variability < 0.2);
...
exports.TemporalEchoEngine = TemporalEchoEngine;
/**
 * Export singleton factory
 */
function createTemporalEchoEngine(config) {
    return new TemporalEchoEngine(config);
}
//# sourceMappingURL=TemporalEchoEngine.js.map