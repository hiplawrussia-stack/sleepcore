...
// TherapyCore type imported from ClinicalContent.ts (CLAUDE.md §13.4)
...
// ICoreSession interface imported from ClinicalContent.ts (CLAUDE.md §13.4)

// CORE_SESSIONS imported from ClinicalContent.ts (CLAUDE.md §13.4 - centralized clinical content)
...
${sonya.tip('Устройтесь поудобнее. Эта сессия заложит важную основу для улучшения вашего сна.')}
  ...
  private async showCoreContent(
    ...
    // Get detailed content for this core (from centralized ClinicalContent.ts)
    // For SRT session (sleep_behavior_1), add personalized sleep window
    let content = getCoreContent(core.id);
...
${formatter.tip('Прочитайте материал внимательно. Затем перейдите к практическому упражнению.')}
  ...
  private async showCoreExercise(
    ...
    // Get interactive exercise for this core (from centralized ClinicalContent.ts)
    const exercise = getCoreExercise(core.id);
...
${disclaimer}
  ...
  // getCoreContent and getCoreExercise now imported from ClinicalContent.ts
  // per CLAUDE.md §13.4 - centralized clinical content
